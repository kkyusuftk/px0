...
- **Reveal Search Results in the Explorer**: Opening a file from file search or a workspace search result expands its parent folders, selects it, and scrolls it into view in the sidebar. Disable this with `explorer.autoReveal`. If the changed-files filter hides the result, it is cleared to reveal the file.
